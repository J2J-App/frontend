...
const CutoffPage = () => {
  ...
  const defaultCategory =
    currentCounselling?.categories?.[0]?.value || "General";
  const defaultSubCategory =
    currentCounselling?.subCategories?.[0]?.value || " ";
  ...
  const fetchCutoffData = async (selectedYear: string) => {
    ...
    try {
      ...
      const response = await fetch(`${API_URL}/v2/cutoff`, {
        method: "POST",
      ...
      <div style={{ zIndex: -1 }}>
        {isLoading ? (
          ...
          <p style={{ color: "white", textAlign: "center" }}>
            No data available for {year}. Select a different year or check your
            filters.
          </p>
        )}
      </div>
