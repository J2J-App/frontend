...
export default function Page() {
  const [collegeData, setCollegeData] = useState<
    Record<string, { logo: string; "college-pic": string }>
  >({});
  ...
  useEffect(() => {
    if (!isLoading) {
      ...
      if (targetResult.success && targetResult.data) {
        const targetSlug = targetResult.data;
        ...
        requestAnimationFrame(() => {
          ...
          const removeResult = safeRemoveLocalStorage("scrollTarget");
          if (!removeResult.success) {
            console.warn("Could not remove scroll target:", removeResult.error);
          }
        });
      } else if (!targetResult.success) {
        console.warn("Could not get scroll target:", targetResult.error);
      }
    }
  }, [isLoading]);
  ...
  useEffect(() => {
    ...
    if (
      !Array.isArray(collegeSequence[counselling]) &&
      collegeSequence[counselling]
    ) {
      setSelectedUniType(
        Object.keys(collegeSequence[counselling] as object)[0]
      );
    }
  }, [counselling]);
          ...
          <div style={{ marginTop: "10px" }}>
            {Array.isArray(collegeSequence[counselling]) ? (
              <div className={Styles.page}>
                {" "}
                {collegeSequence[counselling]
                  .filter((collegeSlug: string) =>
                    getDisplayName(collegeSlug)
                      .toLowerCase()
                      .includes(searchQuery.toLowerCase())
                  )
                  .map((collegeSlug: string) => {
              ...
              <div>
                <div className={Styles.selectContainer}>
                  <SelectMenu
                    options={Object.keys(
                      collegeSequence[counselling] as object
                    ).map((type) => ({
                      value: type,
                ...
                <div className={Styles.page}>
                  {selectedUniType &&
                    (collegeSequence[counselling] as Record<string, string[]>)[
                      selectedUniType
                    ]
                      ?.filter((collegeSlug: string) =>
                        getDisplayName(collegeSlug)
